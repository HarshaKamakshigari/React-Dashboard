...
//           <Command className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground dark:text-[#A0AEC0]" />
...
export function AppHeader({ title, breadcrumb = ["Dashboards"], onNotificationClick }: AppHeaderProps) {
  return (
    <header className="flex h-16 shrink-0 items-center gap-2 md:gap-4 border-b px-4 md:px-6 dark:bg-[#0A0A0A] dark:border-[#2D3748]">
      <SidebarTrigger className="-ml-1 dark:text-[#FFFFFF]" />
      <div className="hidden md:block h-4 w-px bg-border dark:bg-[#2D3748]" />
      
      {/* Breadcrumb: hidden on xs/sm, shown on md+ */}
      <nav className="hidden md:flex items-center gap-2 text-sm text-muted-foreground dark:text-[#A0AEC0] truncate max-w-xs md:max-w-md lg:max-w-lg">
        {breadcrumb.map((item, index) => (
          <React.Fragment key={item}>
            {index > 0 && <span>/</span>}
            <span className={index === breadcrumb.length - 1 ? "text-foreground dark:text-[#FFFFFF]" : ""}>
      ...
      <div className="flex items-center gap-1 md:gap-4">
        {/* Search: full on md+, smaller width on mobile */}
        <div className="relative flex-1 max-w-full md:max-w-xs">
          <Command className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground dark:text-[#A0AEC0]" />
          <Input 
            placeholder="Search" 
            className="pl-9 w-full bg-white dark:bg-[#1A1A1A] dark:text-[#FFFFFF] dark:border-[#2D3748]" 
            aria-label="Search"
          />
        </div>

        {/* Icons: smaller gap on mobile */}
        <Button variant="ghost" size="icon" className="dark:text-[#FFFFFF] dark:hover:bg-[#4C51BF]">
          <Grid3X3 className="h-4 w-4" />
        </Button>
        <ThemeToggle />
